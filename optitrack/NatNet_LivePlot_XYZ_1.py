...
if __name__ == "__main__":
    ...
    # Store time and z-position (or any other measurement)
    max_points = 100
    timestamps = deque(maxlen=max_points)
    x_positions = deque(maxlen=max_points)
    y_positions = deque(maxlen=max_points)  
    z_positions = deque(maxlen=max_points)


    # Listener gets called every frame
    def listener(timestamp, labeled_marker_buffer, rigid_body_buffer):
        print("Full rigid body buffer:", rigid_body_buffer)
        if rigid_body_buffer:
            _, pos, _ = rigid_body_buffer[0]
            print("Position:", pos)
            x, y, z = pos
    ...
    ax.set_title("Live Position of First Rigid Body")
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Position (m)")
    ax.legend()
    ax.set_ylim(-1, 2)  # Set range based on expected motśion
    ax.grid()
    ...
    def update(frame):
        if len(timestamps) > 1:
            ...
            t -= t[0]  # make time relative
            line_x.set_data(t, x_positions)
            line_y.set_data(t, y_positions)
            line_z.set_data(t, z_positions)
